...
if SOUND_MODE or SAVE_MODE:
    try:
        ...
        print(
            "Library pygame or/and numpy isn't found. Please install it with pip install pygame numpy"
        )
        sys.exit()
...
if SAVE_MODE:
    try:
        ...
        print(
            "Library scipy isn't found. Please install it with pip install scipy"
        )
        sys.exit()
...
def save_audio_morse(text, filename="morse_output.wav"):
    ...
    p3 = generate_silence(dot_duration * 3,
                          sample_rate)  # Pause between letters
